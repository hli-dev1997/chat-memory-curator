...
public enum PromptTemplateEnum {
    ...
    /**
     * L2 阶段针对包含图片的会话片段的多模态图文关联性判定提示词
     */
    L2_MULTIMODAL_SESSION_SPLIT(
            "L2_MULTIMODAL_SESSION_SPLIT",
            """
            你是一个专业的 AI 多模态对话上下文关联性分析专家。
            你的任务是分析两段相邻的对话片段（Chunk A 与 Chunk B，包含文本正文及关联的图片），综合看图识图与文本意图，判断它们是否属于同一个对话主题/意图上下文。

            【图文多模态判定规则】：
            1. 应当合并 (MERGE - related: true)：
               - Chunk B 中的图片是对 Chunk A 讨论主题的实物展示、架构图、代码截图或报错截图。
               - Chunk B 中的文本对 Chunk A 中的图片内容进行了提问、解释或延伸。
               - 两者在图文综合语义上处于同一个技术探讨或意图脉络中。

            2. 应当切断 (SPLIT - related: false)：
               - Chunk B 中的图片与 Chunk A 的文本/图片主题毫无关系（例如从“MySQL架构图”跳跃到“美食图片”或“无关网页截图”）。
               - 图文话题主体发生根本性转移。

            3. 模糊与边界决策原则：
               - 若关联证据不够明确，【优先判定为 MERGE (合并保留)】。
            ...
            注意：片段中关联的图片已通过多模态通道输入，请综合分析两段文本与内嵌图片内容，判断 Chunk B 是否与 Chunk A 属于同一会话上下文，并按指定 JSON 格式输出判定结果。
            """,
            "L2 图文多模态千问会话关联性判定提示词模板",
            "v1.0"
    );
